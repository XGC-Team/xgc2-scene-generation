...
ObstacleBase::ObstacleBase(ros::NodeHandle& nh,
    ...
    : nh_(nh),
      config_(config),
      velocity_timeout_(velocity_timeout),
    ...
    if (config_.velocity_obstacle_enabled && !config_.velocity_topic.empty()) {
        velocity_sub_ = nh_.subscribe(
            config_.velocity_topic, 1, &ObstacleBase::velocityCallback, this);

        ROS_INFO("Obstacle [%s]: Subscribed to velocity topic: %s",
                 config_.name.c_str(), config_.velocity_topic.c_str());
    }
    ...
    if (velocity_update_rate > 0 && config_.velocity_obstacle_enabled) {
        velocity_update_timer_ = nh_.createTimer(
            ros::Duration(1.0 / velocity_update_rate),
            [this](const ros::TimerEvent&) { this->updatePosition(); });
    }

    ROS_INFO("Obstacle [%s] (ID: %d, Type: %s) initialized",
             config_.name.c_str(), config_.id, config_.type.c_str());
...
convex_geometry::ConvexBodyInstance ObstacleBase::buildConvexBodyInstance(bool is_static) const {
    convex_geometry::ConvexBodyInstance instance;
...
Eigen::Quaterniond ObstacleBase::currentOrientationQuaternion() const {
    return Eigen::Quaterniond(
        ...
        current_pose_.orientation.z).normalized();
}
