...
std::string
memberString(XmlRpc::XmlRpcValue& xml, const std::string& key, const std::string& default_value) {
...
double memberDouble(XmlRpc::XmlRpcValue& xml, const std::string& key, double default_value) {
    if (!xml.hasMember(key)) {
        return default_value;
    }
...
int memberInt(XmlRpc::XmlRpcValue& xml, const std::string& key, int default_value) {
    if (!xml.hasMember(key)) {
        return default_value;
    }
...
Eigen::Vector3d memberVector3(XmlRpc::XmlRpcValue& xml, const std::string& key) {
    if (!xml.hasMember(key) || xml[key].getType() != XmlRpc::XmlRpcValue::TypeArray ||
        xml[key].size() != 3) {
        throw std::runtime_error("Field '" + key + "' must be [x, y, z]");
    }
    return Eigen::Vector3d(
        xmlToDouble(xml[key][0]), xmlToDouble(xml[key][1]), xmlToDouble(xml[key][2]));
}
...
Eigen::Vector4d memberColor(XmlRpc::XmlRpcValue& xml,
    ...
    if (xml[key].getType() != XmlRpc::XmlRpcValue::TypeArray || xml[key].size() != 4) {
        throw std::runtime_error("Field '" + key + "' must be [r, g, b, a]");
    }
    return Eigen::Vector4d(xmlToDouble(xml[key][0]),
...
Eigen::Quaterniond yawQuaternion(double yaw) {
    ...
    return Eigen::Quaterniond(std::cos(half_yaw), 0.0, 0.0, std::sin(half_yaw));
}
...
void ScenarioManager::printSummary() const {
    ...
    for (const auto& obs : obstacles_) {
        ROS_INFO(
            "  - [%d] %s (Type: %s)", obs->getId(), obs->getName().c_str(), obs->getType().c_str());
    }
    ROS_INFO("========================\n");
}
...
void ScenarioManager::initializeMarkerPublishing() {
    marker_array_pub_ =
        nh_.advertise<visualization_msgs::MarkerArray>("/cluttered_environment/markers", 10);

    if (publish_rate_ > 0) {
        marker_timer_ = nh_.createTimer(
            ros::Duration(1.0 / publish_rate_), &ScenarioManager::publishMarkers, this);
    }
}

void ScenarioManager::initializeDynamicObstaclePublishing() {
    dynamic_body_instances_pub_ = nh_.advertise<xgc2_geometry_msgs::ConvexBodyArray>(
        "/xgc2_geometry/dynamic_body_instances", 10, true);
    ...
    if (has_dynamic) {
        dynamic_instance_timer_ = nh_.createTimer(
            ros::Duration(1.0 / 50.0), &ScenarioManager::publishDynamicBodyInstances, this);
...
bool ScenarioManager::loadObstacles() {
    ...
    auto append_obstacle = [&](const ObstacleConfig& config,
        ...
        if (!obstacle_ids.insert(config.id).second) {
            ROS_ERROR("Duplicate obstacle id %d while loading %s", config.id, source_name.c_str());
            return false;
        }
        ...
        if (!obstacle) {
            ROS_ERROR("Failed to create obstacle id=%d from %s", config.id, source_name.c_str());
            return false;
        }
        ...
        for (int i = 0; i < groups_xml.size(); ++i) {
            try {
                ...
                for (const auto& config : expanded_configs) {
                    if (!append_obstacle(config, "obstacle_groups[" + std::to_string(i) + "]")) {
                        return false;
                    }
                }
...
ObstacleConfig ScenarioManager::parseObstacleConfig(XmlRpc::XmlRpcValue& obs_xml) {
    ...
    config.orientation = Eigen::Quaterniond(static_cast<double>(ori[3]),
    ...
    if (obs_xml.hasMember("geometry") && obs_xml["geometry"].hasMember("scale")) {
        ...
        config.scale = Eigen::Vector3d(static_cast<double>(scale[0]),
    ...
    if (config.velocity_obstacle_enabled &&
        obs_xml["velocity_obstacle"].hasMember("initial_velocity")) {
        ...
        if (init_vel.getType() == XmlRpc::XmlRpcValue::TypeArray && init_vel.size() == 3) {
            config.initial_velocity = Eigen::Vector3d(static_cast<double>(init_vel[0]),
    ...
    if (obs_xml.hasMember("visualization") && obs_xml["visualization"].hasMember("color")) {
        ...
        if (color.getType() == XmlRpc::XmlRpcValue::TypeArray && color.size() == 4) {
            config.color = Eigen::Vector4d(static_cast<double>(color[0]),
    ...
    if (config.type == "h_polytope") {
        if (obs_xml["geometry"].hasMember("h_polytope_type")) {
            expandHPolytopeTemplate(
                static_cast<std::string>(obs_xml["geometry"]["h_polytope_type"]), config);
        } else if (obs_xml["geometry"].hasMember("h_matrix")) {
            parseInlineHMatrix(obs_xml["geometry"]["h_matrix"], config);
        } else {
            throw std::runtime_error(
                "H-polytope obstacle must have either 'h_polytope_type' or 'h_matrix'");
        }
    }

    if (config.type == "v_polytope") {
        if (obs_xml["geometry"].hasMember("v_polytope_type")) {
            expandVPolytopeTemplate(
                static_cast<std::string>(obs_xml["geometry"]["v_polytope_type"]), config);
        } else if (obs_xml["geometry"].hasMember("vertices")) {
            parseInlineVertices(obs_xml["geometry"]["vertices"], config);
        } else {
            throw std::runtime_error(
                "V-polytope obstacle must have either 'v_polytope_type' or 'vertices'");
        }
    }
...
std::vector<ObstacleConfig> ScenarioManager::expandObstacleGroup(XmlRpc::XmlRpcValue& group_xml) {
...
std::vector<ObstacleConfig> ScenarioManager::expandCubeFrameGroup(XmlRpc::XmlRpcValue& group_xml) {
    const int start_id = memberInt(group_xml, "start_id", -1);
    if (start_id < 0) {
        throw std::runtime_error("cube_frame group requires nonnegative start_id");
    }

    const std::string name_prefix = memberString(group_xml, "name_prefix", "cube_frame");
    const std::string motion_type = memberString(group_xml, "motion_type", "static");
    ...
    const double rod_thickness = memberDouble(group_xml, "rod_thickness", 0.25);
    const Eigen::Vector4d color =
        memberColor(group_xml, "color", Eigen::Vector4d(0.50, 0.50, 0.50, 1.00));

    if (width <= 0.0 || height <= 0.0 || rod_thickness <= 0.0) {
        throw std::runtime_error("cube_frame width, height, and rod_thickness must be positive");
    }
...
void ScenarioManager::expandHPolytopeTemplate(const std::string& template_name,
                                              ObstacleConfig& config) {
...
void ScenarioManager::parseInlineHMatrix(XmlRpc::XmlRpcValue& h_matrix_xml,
                                         ObstacleConfig& config) {
...
void ScenarioManager::expandVPolytopeTemplate(const std::string& template_name,
                                              ObstacleConfig& config) {
...
void ScenarioManager::parseInlineVertices(XmlRpc::XmlRpcValue& vertices_xml,
                                          ObstacleConfig& config) {
...
xgc2_geometry_msgs::GeometryLibrary ScenarioManager::buildGeometryLibrary() {
...
xgc2_geometry_msgs::ConvexBodyArray ScenarioManager::buildConvexBodyArray() {
    xgc2_geometry_msgs::ConvexBodyArray array;
    ...
    for (const auto& obstacle : obstacles_) {
        ...
        xgc2_geometry_msgs::ConvexBodyInstance instance = obstacle->buildConvexBodyInstance(true);
        array.instances.push_back(instance);

        ROS_INFO("Added static instance: id=%d, name=%s, type=%s, motion_type=%s, "
                 "scale=[%.2f,%.2f,%.2f]",
                 instance.id,
                 instance.name.c_str(),
                 instance.geometry_type.c_str(),
                 config.motion_type.c_str(),
                 instance.scale.x,
                 instance.scale.y,
                 instance.scale.z);
...
void ScenarioManager::publishGeometryOnce() {
    ...
    geometry_library_pub_ = nh_.advertise<xgc2_geometry_msgs::GeometryLibrary>(
        "/xgc2_geometry/geometry_library", 1, true);

    static_body_instances_pub_ = nh_.advertise<xgc2_geometry_msgs::ConvexBodyArray>(
        "/xgc2_geometry/static_body_instances", 1, true);

    ros::Duration(0.5).sleep();

    xgc2_geometry_msgs::GeometryLibrary library = buildGeometryLibrary();
    geometry_library_pub_.publish(library);

    xgc2_geometry_msgs::ConvexBodyArray instances = buildConvexBodyArray();
