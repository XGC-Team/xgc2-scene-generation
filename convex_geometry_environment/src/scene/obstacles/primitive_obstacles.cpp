...
void SphereObstacle::generateGeometry() {
    bbox_min_local_ = Eigen::Vector3d(
        -radius_ * config_.scale.x(),
        -radius_ * config_.scale.y(),
        -radius_ * config_.scale.z());
...
convex_geometry::GeometryTemplate SphereObstacle::buildGeometryTemplate(int /*resolution*/) const {
...
convex_geometry::ConvexBody SphereObstacle::buildOccupiedBody() const {
    ...
    body.shape = std::make_shared<convex_geometry::SphereSet>(body.position, body.scale.x());
    return body;
}
...
convex_geometry::GeometryTemplate CylinderObstacle::buildGeometryTemplate(int resolution) const {
    convex_geometry::GeometryTemplate tmpl;
    tmpl.type = getType();
    ...
    for (int i = 0; i < tmpl.resolution; ++i) {
        ...
        tmpl.support_points.push_back(toGeometryPoint(Eigen::Vector3d(std::cos(angle), std::sin(angle), -0.5)));
        tmpl.support_points.push_back(toGeometryPoint(Eigen::Vector3d(std::cos(angle), std::sin(angle), 0.5)));
    }
...
convex_geometry::ConvexBody CylinderObstacle::buildOccupiedBody() const {
    convex_geometry::ConvexBody body = makeBodyMetadata(
        config_,
        getType(),
        currentPositionVector(),
        ...
        config_.scale,
        currentVelocityVector());
    body.shape = std::make_shared<convex_geometry::CylinderSet>(
...
convex_geometry::GeometryTemplate CapsuleObstacle::buildGeometryTemplate(int /*resolution*/) const {
    convex_geometry::GeometryTemplate tmpl;
    tmpl.type = getType();
    tmpl.resolution = 0;
    return tmpl;
...
convex_geometry::ConvexBody CapsuleObstacle::buildOccupiedBody() const {
    convex_geometry::ConvexBody body = makeBodyMetadata(
        config_,
        getType(),
        currentPositionVector(),
        ...
        config_.scale,
        currentVelocityVector());
    body.shape = std::make_shared<convex_geometry::CapsuleSet>(
        body.position, body.scale.x(), body.scale.z(), body.orientation);
    return body;
}
...
void CubeObstacle::generateGeometry() {
    ...
    const Eigen::Vector3d hs(
        half_size.x() * config_.scale.x(),
...
convex_geometry::GeometryTemplate CubeObstacle::buildGeometryTemplate(int /*resolution*/) const {
    convex_geometry::GeometryTemplate tmpl;
    tmpl.type = getType();
    tmpl.resolution = 0;
...
convex_geometry::ConvexBody CubeObstacle::buildOccupiedBody() const {
    convex_geometry::ConvexBody body = makeBodyMetadata(
        config_,
        getType(),
        currentPositionVector(),
        ...
        config_.scale,
        currentVelocityVector());
    body.shape = std::make_shared<convex_geometry::BoxSet>(
